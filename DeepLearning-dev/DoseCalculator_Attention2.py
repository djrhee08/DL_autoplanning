...
class PerCPProjectionLayer(nn.Module):
    """Projects each CP's BEV feature map into the 3D bottleneck volume (48³)
    ...
    Output: beam_stack    [B, N_cp, 32,  48,  48, 48]
    """
    def __init__(self, average=False, vol_size=(48, 48, 48)):
        super().__init__()
    ...
    def forward(self, bev_features):
        ...
        return projected.view(B, N, C, D, H_vol, W_vol)     # [B, N_cp, C, 48, 48, 48]
...
class SpatialBeamAttention(nn.Module):
    ...
    For each voxel in the 48³ bottleneck, computes a soft attention weight
    ...
    Args:
        ct_channels   : channels of the CT bottleneck (64)
    ...
    """
    def __init__(self, ct_channels=64, beam_channels=32, num_cps=179):
        super().__init__()
...
class VMATDosePredictorAttention(nn.Module):
    ...
    Data flow (average=True, N_cp=179):
        BEV apertures [B, 179, 2, 560, 560]   MU [B, 179]
            ...
            → PerCPProjectionLayer   → [B, 179, 32,  48,  48, 48]
            × mu[:, :, None, None, None, None]   ← MU scales 3D volumes
                                                      ↓
        CT [B,1,192³] → U-Net encoder → bottleneck [B,64,48³]
                                                      ↓
                          SpatialBeamAttention  →  fused [B, 32, 48³]
                                                      ↓
                     concat + fusion_conv       →  [B,64,48³]
                                                      ↓
    ...
    def __init__(self, average=True):
        ...
        self.per_cp_proj = PerCPProjectionLayer(average=average, vol_size=(48, 48, 48))
        self.beam_attn   = SpatialBeamAttention(ct_channels=64, beam_channels=32, num_cps=num_cps)

        # Merge attended beam features into bottleneck channels (64 + 32 = 96)
        self.fusion_conv = Conv3DBlock(96, 64)

        # --- CT encoder (U-Net, 2 downsampling steps: 192→96→48) ---
        ...
        self.bottleneck = Conv3DBlock(64, 64)                      # [B,  64,  48³]
    ...
    def forward(self, ct, bev_apertures, mu):
        ...
        # --- 1. Beam pathway ---
        bev_feat   = self.bev_encoder(bev_apertures)            # [B, N_cp, 32, 70, 70]
        beam_stack = self.per_cp_proj(bev_feat)                 # [B, N_cp, 32, 48, 48, 48]
